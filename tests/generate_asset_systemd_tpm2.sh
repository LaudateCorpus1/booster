trap 'swtpm_ioctl --tcp :2322 -s; rm cryptenroll.passphrase; sudo umount $dir; rm -r $dir; sudo cryptsetup close $LUKS_DEV_NAME; sudo losetup -d $lodev' EXIT
trap 'rm $OUTPUT' ERR
...
sudo CREDENTIALS_DIRECTORY="$(pwd)" systemd-cryptenroll --tpm2-device=swtpm: --tpm2-pcrs=10+13 $lodev
